...
def test_debt_documented_formula(value, principal, interest, risk):
    inputs = DebtInputs(**dict.fromkeys(DEBT_FEATURES, value), evidence_confidence=1.0)
...
def test_debt_unequal_features_and_explainable_contributions():
    score = score_debt(
        DebtInputs(
            complexity=0.8,
...
def test_missing_debt_feature_lowers_confidence_and_never_increases_priority(missing):
    known = dict.fromkeys(DEBT_FEATURES, 0.5) | {"evidence_confidence": 0.9}
    complete = score_debt(DebtInputs(**known))
    score = score_debt(DebtInputs(**(known | {missing: None})))
...
def test_all_missing_is_distinct_from_measured_zero():
    ...
    assert score_remediation(RemediationInputs(**SAFE_PATCH)).confidence == 1


@pytest.mark.parametrize("risk_value", [0.0, 0.5, 1.0])
def test_remediation_normal_and_boundary_cases(risk_value):
...
def test_each_patch_feature_has_expected_direction_and_weight(feature, weight):
    values = SAFE_PATCH | {feature: 1 - SAFE_PATCH[feature]}
    score = score_remediation(RemediationInputs(**values))
...
def test_migrations_cannot_be_trivially_reversible(feature, value, expected):
    score = score_remediation(RemediationInputs(**(SAFE_PATCH | {feature: value})))
...
def test_missing_remediation_feature_is_explicit_and_conservative(feature):
    score = score_remediation(RemediationInputs(**(SAFE_PATCH | {feature: None})))
...
def test_context_and_evidence_confidence_penalties(context, evidence, expected):
    score = score_remediation(
        RemediationInputs(
            **(
                SAFE_PATCH
...
def test_invalid_normalized_values_are_rejected(model, fields, invalid):
...
def test_input_contract_rejects_unknown_fields_and_versions(model):
    with pytest.raises(ValidationError):
...
def test_scores_are_deterministic_and_json_round_trip():
    debt = DebtInputs(**dict.fromkeys(DEBT_FEATURES, 0.5), evidence_confidence=0.9)
    patch = RemediationInputs(**SAFE_PATCH)
    for scorer, inputs, output_model in [
...
def test_debt_does_not_change_when_patch_risk_changes():
    inputs = DebtInputs(**dict.fromkeys(DEBT_FEATURES, 0.5), evidence_confidence=1.0)
    before = score_debt(inputs)
    safe = score_remediation(RemediationInputs(**SAFE_PATCH))
    risky = score_remediation(RemediationInputs(**(SAFE_PATCH | {"blast_radius": 1.0})))
