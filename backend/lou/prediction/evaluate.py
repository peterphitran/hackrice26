...
def evaluate_impact(prediction: ImpactPrediction, observed: ObservedImpact) -> ImpactEvaluation:
    ...
    labels.update({f"{kind}:{key}": "false_negative" for kind, key in sorted(actual.keys() - predicted.keys())})
    ...
    return ImpactEvaluation(
        analysis_run_id=prediction.analysis_run_id, repository_id=prediction.repository_id,
        predictor_revision=prediction.predictor_revision, labels=labels, precision=precision,
        recall=recall, false_negative_rate=1 - recall,
